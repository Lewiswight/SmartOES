...
from devices.xbee.common.prodid import LYNXEE
import time
import thread
...
class XBeeSerialTerminal(XBeeSerial):
    ...
    def __init__(self, name, core_services):
        ...
        property_list = [
            ...
            ChannelSourceDeviceProperty(name="r_t_ch", type=str,
                  initial=Sample(timestamp=0, unit="", value="0"),
                  perms_mask=(DPROP_PERM_GET|DPROP_PERM_SET),
                  options=DPROP_OPT_AUTOTIMESTAMP,
                  set_cb=lambda x: self.update_local("r_t_ch", x)),
            
            ChannelSourceDeviceProperty(name="r_t_on", type=Boolean,
                initial=Sample(timestamp=0,
                value=Boolean(False, style=STYLE_ONOFF)),
                perms_mask=(DPROP_PERM_GET|DPROP_PERM_SET),
                options=DPROP_OPT_AUTOTIMESTAMP,
                set_cb=lambda x: self.remote_temp_loop("r_t_on", x)),
    ...
    def remote_temp_loop(self):
        
        on = self.property_get(r_t_on).value 
        sensor = self.property_get(r_t_ch).value
        
        if sensor == "0":
            self.property_set_globe("r_t_on", Sample(0, value=Boolean(bool(0), style=STYLE_ONOFF)))
            print "no channel name"
        
        
        while on and sensor != "0":
            cm = self.__core.get_service("channel_manager")
            cdb = cm.channel_database_get()
            cp = cm.channel_publisher_get()
            channel = cdb.channel_get(sensor + ".temperature") 
            sample1 = channel.get()
            
            temp = sample1.value
            
            try: 
                self.serial_send("CT=" + temp)
            except: 
                print "could not send CT"
    ...
    def remote_temp(self, register_name, val):
        
        self.property_set(register_name, val)
        
        val = self.property_get(register_name).value    
        
        if val and self.remote_temp_l == 0:
            thread.start_new_thread(self.remote_temp_loop) 
            
        
        
        
    
    def update_local(self, register_name, val):
