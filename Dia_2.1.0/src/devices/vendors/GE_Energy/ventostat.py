...
* **sample_rate_sec:** Rate at which to sample the ventostat in seconds.
...
YML Example:
  ...
  - name: upstairs
    driver: devices.vendors.RCS.xbee_RCS:XBeeRCS
    settings:
...
class Ventostat(RobustXSerial):
    ...
    def __init__(self, name, core_services):
        ...
        settings_list = [
            ...
            # enable the LED statistics channels
            Setting(
    ...
    # use base-class's apply_settings(self):
    ...
    def update_stats_c02(self, val, now, now_st=None):
        if self.__co2_stats is not None:
            ...
            if now_st is not None:
                msg = 'CO2, min=%d, avg=%d, max=%d, as of %s' % \
    ...
    def update_stats_humidity(self, val, now, now_st=None):
        if self.__hum_stats is not None:
            ...
            if now_st is not None:
                msg = 'HUM, min=%0.1f, avg=%0.1f, max=%0.1f, as of %s' % \
    ...
    def update_stats_temperature(self, val, now, now_st=None):
        if self.__tmp_stats is not None:
            ...
            if now_st is not None:
                msg = 'TMP, min=%0.1f, avg=%0.1f, max=%0.1f, as of %s' % \
